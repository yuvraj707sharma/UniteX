-- ============================================
-- COMMUNITY UPGRADE - Production Grade
-- ============================================
...
-- Add created_by alias (maps to creator_id)
-- Don't need to add, just use creator_id as created_by

-- STEP 2: Create community_roles table for role management
...
-- STEP 3: Create community_posts table (separate from main posts)
...
-- STEP 7: Create RLS Policies for community_roles
...
-- STEP 8: Create RLS Policies for community_posts
...
-- STEP 9: Create RLS Policies for community_post_likes
...
-- STEP 10: Create RLS Policies for community_post_comments
...
CREATE POLICY "Authors and admins can delete comments"
  ...
  USING (
    ...
    EXISTS (
      ...
      JOIN public.community_roles cr ON cr.community_id = cp.community_id
...
-- STEP 13: Create triggers for auto-updating counts
...
-- STEP 14: Auto-assign creator as admin when community is created
CREATE OR REPLACE FUNCTION assign_creator_as_admin()
RETURNS TRIGGER AS $$
BEGIN
  -- Insert creator into community_roles as admin
...
-- STEP 15: Function to get user role in community
...
SELECT 
  ...
  (SELECT COUNT(*) FROM information_schema.tables WHERE table_name IN ('community_roles', 'community_posts', 'community_post_likes', 'community_post_comments')) as new_tables_created;

-- STEP 17: Sample data for testing (optional - uncomment to use)
/*
-- Add username to existing communities
UPDATE public.communities
SET username = 'community_' || SUBSTRING(id::text, 1, 8)
WHERE username IS NULL;

-- Add some sample tags
UPDATE public.communities
SET tags = ARRAY['technology', 'social']
